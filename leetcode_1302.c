...
//使用 c 語言函式庫 qsort 裡呼叫 cmp 函數去比較兩數的大小,qsort 函數會在幫我們把每一列的數字和去做排序
...
long long kthLargestLevelSum(struct TreeNode* root, int k) {
        TreeNode* que[100001] = { NULL }; //依據題目要求 10^5,所以要把 array 設定成這個大小
        long long ans[100001] = { 0 };
        //head 跟 tail 是作為 queue 的索引作為放入放入 queue 的索引,cnt 是用來數當前的層數如果處理完第一層則往下一層時 cnt 增加 1,size 則是作為當前層數有多少節點,作為 while 裡的參數來決定要做幾次 while 迴圈
        int head = 0, tail = -1, cnt = 0, size = 0;
        if(root != NULL) que[++tail] = root;
        //root 不為 NULL 放入第一個在 queue 裡第 0 位為 root
        while(head <= tail) {
                size = tail - head + 1;
                //透過 head 跟 tail 的索引來尋找當前總共有多少節點,由於 tail 跟 head 不會重置則每次在迴圈新增 array 裡的資料時 tail 會一起增加透過這樣的方式可以計算出當前層數有多少節點
                while(size-- > 0) {
                        TreeNode* cur = que[head++];
                        //把 while 迴圈裡的當前已被放入資料給 cur 指針
                        ans[cnt] += cur->val;
                        //當前層數存放答案,透過每一次 while 放入前一次處理的數並累加,直到離開 while 並 cnt++後到下一層
